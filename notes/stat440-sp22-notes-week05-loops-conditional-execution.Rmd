...
Summarization occurs when a function or operation takes a vector as input, completes the function or operation, and returns a single value (effectively a summarized value). We discuss summarizing data in Week 07.
...
Working with an old version of the City of Urbana's [Rental Inspection Grades Listing Data](https://uofi.box.com/shared/static/39erszqhkhgom7fnzgef7ufnkde3os0m.json), we can discuss how to get the information we want: the 1730 rows and 7 columns about the rental properties. One sure-way to do this is to use a `for` loop and we can combine this idea with vectorized function `ifelse()`.

```{r}
library(tidyverse)
library(jsonlite)
rd <- fromJSON("https://uofi.box.com/shared/static/39erszqhkhgom7fnzgef7ufnkde3os0m.json")
names(rd)
rd2 <- rd$data
length(rd2)
...
rd3 <- tibble(property_address="",parcel_number="",inspection_date="",grade="",license_status="",expiration_date="",mappable_address="",.rows=1738)
