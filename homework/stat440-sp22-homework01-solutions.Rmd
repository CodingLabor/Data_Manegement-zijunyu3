...
**Answer:** Answers will vary, but must write at least one of the choices. Graded for completeness.
...
**Answer:** Answers will vary, but must have 3 complete sentences. Graded for completeness.
...
**Answer:** Answers will vary, but must be two lists each with three items. Graded for completeness. For example,

List 1

a. Data is a picture of a black man grilling food.  
...
```{r prob07}
rentals <- read_csv("https://uofi.box.com/shared/static/61w13855u06z11007t7nypo710gbiw4b.json")
str(rentals)
```
...
prop_url <- paste0("https://champaignil.devnetwedge.com/parcel/view/",rentals$`Parcel Number`,"/2021")

sales <- rep(0, length(prop_url))

for (i in 1:10){ 
  if(length(html_text(html_nodes(read_html(prop_url[i]), "#parcel-views :nth-child(1) td:nth-child(9)"),trim=TRUE))==0){
    sales[i] <- NA
  }
    else{
  sales[i] <- html_text(html_nodes(read_html(prop_url[i]),"#parcel-views :nth-child(1) td:nth-child(9)"),trim=TRUE)
    }
}
