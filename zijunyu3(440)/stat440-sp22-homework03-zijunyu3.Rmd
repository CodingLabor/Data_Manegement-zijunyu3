...
**Answer:I had trouble learning new R methods, it is hard for me to understand how these methods work. Besides, I feel nervous about reviewing for Midterm, and I hope I get a great score on exams. I have some friends who are exposed to Covid, so I am kind of worried about if I will get Covid.** 
...
**Answer:HOND and TOYT;BUIC AND CHRY;AUDI,MITS AND SUBA;LINC AND MERZ are tied** 
```{r}
library(readr)
library(tidyverse)
cars <- read_csv("https://uofi.box.com/shared/static/2yfvl36q1th1aq2nvhtiomhsk2uxip6u.csv")
...
sort(table(cars2$Make), decreasing = TRUE)
```
...
**Answer:TAXI driver type has served the longest number of months on average, around 125 months in average**

```{r}
library(tidyverse)
library(readr)
chau <- read_csv("https://uofi.box.com/shared/static/4qcdq6qtofbtn3m7ifrnxaahzv6x1x0r.csv", 
    col_types = cols(`Status Date` = col_date(format = "%m/%d/%Y"), 
        `Original Issue Date` = col_date(format = "%m/%d/%Y")))

chau2 <- chau %>%
  mutate(diff = as.numeric(difftime(`Status Date`,`Original Issue Date`,unit="days"))) %>%
  filter(Status == "ACTIVE")

types <- unique(chau2$`Driver Type`)
avg_diff <- rep(0, length(types))
for (i in 1:length(types)){
  avg_diff[i] <- chau2 %>%
    filter(`Driver Type`==types[i]) %>%
    select(diff) %>%
    colMeans(.,na.rm=TRUE)
}
tibble(types,avg_diff)
...
library(tidyverse)
library(readr)
school <- read_csv("https://uofi.box.com/shared/static/2pd4ibwgbz0m5sojmc02svcihjrsgw5h.csv", 
  col_types = cols(`College Enrollment Rate %` = col_double()))
sum(is.na(school$`College Enrollment Rate %`))

Coordinates00 <- str_remove_all(school$Location, "\\)|\\,|\\(")
Coordinates000 <- str_split(Coordinates00, " ", simplify = TRUE)     

school1 <- school %>%
  mutate(`College Enrollment Potential` = `College Enrollment (number of students)` / (`College Enrollment Rate %` * 0.01) ) %>%
  mutate(Latitude=Coordinates000[,1])%>%
  mutate(Longitude=Coordinates000[,2])%>%
  drop_na(`College Enrollment Rate %`) %>%
  rename(`Average Student Attendance %` = `Average Student Attendance`, 
         `Average Teacher Attendance %` = `Average Teacher Attendance`) %>%
...
slice_head(school1,n=5)

slice_tail(school1,n=5)
...
**Answer:AUSTIN,EAST GARFIELD PARK,HUMBOLDT PARK,NORTH LAWNDALE,NEAR WEST SIDE,SOUTH LAWNDALE,GRAND BOULEVARD,KENWOOD,NEAR NORTH SIDE and WEST TOWN are 10 community area names in chicago with the most crime reports** 
```{r}
library(readr)
...
x <- inner_join(crime, school, by="Ward") 
