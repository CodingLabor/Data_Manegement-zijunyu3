...
**Answer:It is very hard for to me to understand vectorization. I think the labs are getting more difficult, which is not easy to finish in 50 minutes. I got to know more friends in class, so I learned a lot of knowledge and programming from them.** 
...
**Answer:TOYT is the third most common Make among the 4D vehicles were towed.**

```{r prob6}
library(readr)
library(tidyverse)
d4 <- read_csv("https://uofi.box.com/shared/static/2yfvl36q1th1aq2nvhtiomhsk2uxip6u.csv")
...
d4d <- d4 %>%
  ...
  filter(Style == '4D')
print(d4d)
sort(table(d4d$Make), decreasing = TRUE)
```
...
**Answer:MIRANDA, GILBERTO is the longest-serving active public chauffeur in operation in chicago for 68 years**
...
driver2 <- driver %>%
  mutate(diff = difftime(`Original Issue Date`,`Status Date`,unit="days")) %>%
  filter(Status == "ACTIVE", `Chauffeur City` == "CHICAGO")

driver3 <- driver2[order(as.Date(driver2$`Original Issue Date`, format="%m/%d/%Y")),]

#driver3 <- driver2[order(driver2$diff),]

today <- as.Date("2022-02-18")

print(driver3)

#print((driver3$diff[1])/365)

as.numeric(difftime(today, driver3$`Original Issue Date`[1], unit = "days")) / 365
```
...
library(tidyverse)
library(readr)
student <- read_csv("https://uofi.box.com/shared/static/iwdcncwuotz0wv8ax6yv1dbfjdtdrp1y.csv")

student1 <- student %>%
  filter(`11th Grade Average ACT (2011)`!= "NDA",`Elementary, Middle, or High School` == "HS",`Freshman on Track Rate %`>= 80.0)
  

head(student1, 20)

act = mean(as.numeric(student1$`11th Grade Average ACT (2011)`))
