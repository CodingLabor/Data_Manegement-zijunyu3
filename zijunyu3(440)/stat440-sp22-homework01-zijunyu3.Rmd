...
**Answer:Labs are very challenging to finish even with smart peers. It is hard for me to understand the data management just by watching videos, so maybe I need other resources. I also think some R methods are hard to use, so I should get to know more about R language** 
...
- Barbecue seems to be very delicious.
- Dr.Kinson has a great smile.
- This photo was not taken in Winter.

**3 questions about the data**

- What brand is Dr.Kinson's car?
- I wonder What makes Dr.Kinson so happy.
- What size is Dr.Kinson's T-shirt?
...
```{r}
library(jsonlite)
Congress <- fromJSON("https://theunitedstates.io/congress-legislators/legislators-current.json")
str(Congress[[2]])
```
...
```{r}
library(jsonlite)
rental <- fromJSON("https://uofi.box.com/shared/static/lbqe1l9a2g5lr0twk6zcwajasybekget.json")
str(rental)
```
...
Net_total <- rep(0, length(prop_url))
for (i in 1:10){ 
  if(length(html_text(html_nodes(read_html(prop_url[i]), ".text-center:nth-child(1) td:nth-child(9)"),trim=TRUE))==0){
    Net_total[i] <- NA
  }
    else{
  Net_total[i] <- html_text(html_nodes(read_html(prop_url[i]),".text-center:nth-child(1) td:nth-child(9)"),trim=TRUE)
    }
}
head(Net_total,10)
```
...
**#9** (Accessing and importing data, Handling dates and times) Import the Rental Inspection Grades Listing Data using its data URL https://uofi.box.com/shared/static/uxk757wno4yzqyqtrahwzvckjvs9ulmn, print the structure of the data, and ensure the dates and times columns are formatted properly as dates and times in R respectively. This structure should match the data description below.  
