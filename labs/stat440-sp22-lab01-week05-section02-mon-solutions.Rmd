...
#### Only the driver should submit this assignment into their individual student repo. Save the filename as lab01-week05-section02-mon-netID.Rmd and render this file to .html. 
...
- Among the first 10 product categories, what is the average number of varieties for each product category?
...
avg_varieties<- rep(0, length(products))
...
tibble(products,avg_varieties)
```

**#5** (Loops and conditional execution, Apply family of functions, Vectorization) Reshape the imported data in **Problem 3** such that the columns named as the 12 months of the year become one single column named "Months" with their corresponding Yes or No values named as "Availability". Print the first 13 rows of the reshaped data. **You cannot use pivot_wider/longer functions.**
